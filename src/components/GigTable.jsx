...
function GigRow({ gig, past, onEdit, onDelete }) {
  return (
    <tr className={past ? "past" : ""}>
      ...
      <td>
        <PeopleTags names={gig.interested} className="interested" />
      </td>
      <td>
        <PeopleTags names={gig.ticketsBought} className="ticket" />
      </td>
      <td style={{ maxWidth: "140px", fontSize: "0.8rem", color: "var(--text-muted)" }}>{gig.notes || "-"}</td>
      <td>
        {gig.link ? (
          <a href={gig.link} target="_blank" rel="noopener noreferrer" className="gig-link" style={{ marginTop: 0 }}>
            Link
          </a>
        ) : (
          "-"
  ...
  return (
    <div className="gig-table-wrap">
      <table className="gig-table">
        ...
        <tbody>
          {all.map((gig) => (
            <GigRow key={`${gig.rowIndex}-${gig.band}`} gig={gig} past={!upcoming.includes(gig)} onEdit={onEdit} onDelete={onDelete} />
          ))}
        </tbody>
      </table>
    </div>
